...
public class Obligations {
     ...
     * Some things are hardcoded in this example, so there will be errors with more than one PDS.
     * @param graph
     * @param userID
     * @param pdsNode
     * @throws PMException
     */
    public static void createPDS(Graph graph, long userID, Node pdsNode) throws PMException {
        ...
        Node iiNode = graph.createNode(getID(), "InvestigatorInfo", OA, null);
        graph.assign(iiNode.getID(), pdsNode.getID());

        // create the PI container and assign it to InvestigatorInfo
        Node piNode = graph.createNode(getID(), "PI", OA, null);
        graph.assign(piNode.getID(), iiNode.getID());
        ...
        graph.associate(piUA.getID(), pdsNode.getID(), new HashSet<>(Arrays.asList("assign")));
        ...
        graph.associate(spUA.getID(), iiNode.getID(), new HashSet<>(Arrays.asList("read")));
